...
export function NeighborsModerationTable() {
    ...
    const [showOnlyOffenders, setShowOnlyOffenders] = useState(true);
    ...
    const displayedUsers = showOnlyOffenders 
        ? users.filter(u => (u.warning_count || 0) > 0 || u.is_banned) 
        : users;

    return (
        <div className="space-y-4">
            <div className="flex items-center justify-end">
                <label className="flex items-center gap-2 cursor-pointer p-2 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 transition-colors">
                    <input 
                        type="checkbox" 
                        ...
                        {displayedUsers.length === 0 && (
                            <tr>
                                <td colSpan={4} className="px-6 py-12 text-center text-slate-400 font-bold">
                                    {showOnlyOffenders ? 'No hay infractores activos actualmente en el radar.' : 'No hay usuarios registrados.'}
                                </td>
                            </tr>
                        )}
                    </tbody>
