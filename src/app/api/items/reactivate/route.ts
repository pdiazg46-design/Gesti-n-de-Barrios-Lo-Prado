...
export async function POST(request: Request) {
    try {
        ...
        const { data: item } = await supabaseAdmin
            .from('items')
            .select('creator_id, author_email, type')
            .eq('id', itemId)
            .single();
        ...
        if (item.type !== 'CIVIC_REPORT') {
            return NextResponse.json({ error: 'Solo los reportes cívicos pueden reactivarse' }, { status: 400 });
        }
        ...
        if (!isOwner) {
            return NextResponse.json({ error: 'No tienes permiso para reactivar esta publicación' }, { status: 403 });
        }
